# -*- coding: utf-8 -*-
"""Debug runner SPOC project crashing (RCPSP, folder fix/).
...
Perbedaan dari debug_soac_crashing_domain.py lama:
  - import dari project_crashing_problem_fix (folder yang sama)
  - data JSON dibaca dari folder fix/ sendiri (bukan ../adiel/data)
  - mode RCPSP: resource_capacity_v3 + resource_requirements_v3 dimuat
  - _maybe_track_fitness pakai WHITELIST method fitness + sanity check
    F <= 1, supaya output select_final_roots (array durasi, nilai ~30)
    tidak menimpa F_best (bug lama "F_best=30")
"""
import json
import time
...
RES_CAP = current_dir / "resource_capacity_v3.json"
RES_REQ = current_dir / "resource_requirements_v3.json"

DEADLINE = 245

# Hanya method-method ini yang outputnya boleh dianggap nilai fitness.
# Method lain (decode, select_final_roots, makespan, dll.) juga
# mengembalikan array numerik tetapi BUKAN fitness.
_FITNESS_METHODS = {"evaluate_fitness", "g_func", "__call__"}

# Budget penuh -- ~m*(k+1) eval fase cluster, ~sdoa_m*sdoa_k_max per cluster.
PARAMS_FULL = {
    "m_cluster": 2048, "k_cluster": 500, "gamma": 0.85,
    "r_cl": 0.975, "theta_cl": np.pi / 4,
    "sdoa_m": 4096, "sdoa_k_max": 600,
    "sdoa_r": 0.975, "sdoa_theta": np.pi / 8,
...
class DebugProblem:
    ...
    def _maybe_track_fitness(self, name, out):
        """Catat progres; hanya dipanggil untuk method di _FITNESS_METHODS."""
        arr = None
        ...
        try:
            ...
            # Sanity check: F = 1/(1+Z_norm) atau 1/(2+...) selalu di (0, 1].
            # Nilai > 1 berarti output ini bukan fitness -- jangan dicatat.
            if m <= 1.0 and m > self._best_F:
...
def main():
    ...
    res_cap = json.load(open(RES_CAP))
    res_req = json.load(open(RES_REQ))
    problem = ProjectCrashingProblem(
        tasks, DEADLINE, params=params, unit_cube=True,
    ...
    mk_normal = problem.resource_makespan(problem.d_max)
    mk_min = problem.resource_makespan(problem.d_min)
    print(f"{problem.name} | makespan RCPSP normal={mk_normal}, min={mk_min} "
          f"| resource pengikat: "
          f"{[problem.resource_names[i] for i in problem._bind]}")
    ...
    roots = result["roots"]
    print(f"Waktu SPOC: {dt:.1f}s, cluster: {len(result['clusters'])}")
    if len(roots) == 0:
        print("SPOC tidak menemukan solusi feasible.")
        return
    # roots dari select_final_roots sudah berupa vektor durasi d (bukan
    # titik unit-cube), jadi crash_cost/report bisa dipakai langsung.
    costs = [problem.crash_cost(r) for r in roots]
    print(f"SPOC: {len(roots)} solusi, biaya terbaik = {min(costs)}")
