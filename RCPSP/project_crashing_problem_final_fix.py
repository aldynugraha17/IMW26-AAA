...
class ProjectCrashingProblem(BaseProblem):
    def __init__(self, tasks, deadline, params=None, unit_cube=True,
        ...
        super().__init__()
        self.equations = None
        # Batas domain ruang-x (unit cube [0,1]^n bila unit_cube=True, atau
        # domain durasi melebar bila False) -- untuk cek in_cube tervektorisasi
        # di evaluate_fitness, meniru pola `is_in_domain` pysne.
        self._dom_lo = np.array([b[0] for b in self.domain], dtype=float)
        self._dom_hi = np.array([b[1] for b in self.domain], dtype=float)
    ...
    # ---------------- decode ----------------
    def decode(self, x):
        """Transformasi MURNI x (ruang spiral) -> d (durasi integer). Tanpa gerbang.

        Pengecekan domain TIDAK dilakukan di sini, melainkan tiga lapis di
        tempat yang tepat (semua di ruang-d, pasca-pembulatan):

        (1) Fase clustering  : pysne built-in `is_in_domain(x, domain)`
            (modified_clustering_process.py:264) menyaring titik luar unit
            cube dari PEMBENIHAN cluster. Sudah ada, tak perlu ditambah.
            Catatan: di fase SDOA is_in_domain TIDAK dipanggil pysne --
            pelindungnya adalah lapis (2).
        (2) evaluate_fitness : mask tervektorisasi `in_dom` di ruang-d;
            titik luar [d_min, d_max] mendapat F = 0 (bukan kandidat, tak
            pernah memenangkan argmax, tak pernah masuk SGS). Cek ruang-d
            MENSUBSUMSI cek x in [0,1]: x ketat di luar [0,1] pasti decode
            keluar rentang, sedangkan x tepat di tepi (0/1) bisa decode
            keluar akibat banker's rounding np.rint -- hanya cek ruang-d
            yang menangkapnya.
        (3) select_final_roots: kandidat yang decode-nya keluar
            [d_min, d_max] dibuang sebelum feasibility & biaya.

        Mengapa decode tidak boleh jadi gerbang: ia harus SELALU
        mengembalikan d untuk x apa pun (pemanggil yang memutuskan nasib
        titik), dan domain pembandingnya pun beda ruang -- x hidup di
        [0,1]^n, bukan di [d_min-0.5, d_max+0.5].
        """
        ...
        return d          # tanpa clip: titik luar domain ditangani F=0 (Variant B)
    ...
    def evaluate_fitness(self, x):
        ...
        # ---- Lapis 1: cek ruang-x terhadap domain [0,1]^n ----
        # Versi tervektorisasi dari pola pysne
        #   `if not is_in_domain(points[i], domain): continue`
        # (satu mask untuk seluruh batch, bukan loop per titik).
        # Catatan kejujuran teknis: lapis ini REDUNDAN terhadap lapis 2 --
        # x ketat di luar [0,1] terbukti selalu membulat keluar
        # [d_min, d_max], dan kasus tepi x=0/x=1 (banker's rounding) justru
        # hanya tertangkap lapis 2. Ia dipertahankan eksplisit demi
        # kesesuaian dengan pola pysne dan sebagai pengaman bila pemetaan
        # decode kelak berubah.
        in_cube = ((X >= self._dom_lo) & (X <= self._dom_hi)).all(axis=1)

        D = self.decode_batch(X)

        # ---- Lapis 2: pagar domain ruang-d (kontrak Variant B / tanpa clip) ----
        # Tanpa np.clip di decode, titik spiral yang keluar unit cube
        # menghasilkan durasi di luar [d_min, d_max] -- termasuk negatif.
        # Titik seperti itu BUKAN kandidat: F = 0, dan tidak boleh masuk
        # SGS (durasi negatif membuat jendela penjadwalan tak bermakna)
        # maupun rumus F (Z_norm bisa < 0 -> pembagian nol / F > 1 yang
        # menciptakan attractor palsu di luar domain).
        in_dom = in_cube & ((D >= self.d_min) & (D <= self.d_max)).all(axis=1)

        E_cpm = self._forward_pass_batch(D)
        cpm_ms = E_cpm.max(axis=1)
        Z_norm = np.where(in_dom, ((self.d_max - D) @ self.c) / self.Z_max, 0.0)

        if self.has_resources:
            # makespan efektif: CPM (batas bawah) di daerah jauh-infeasible,
            # SGS resource-feasible hanya untuk titik IN-DOMAIN yang lolos CPM.
            eff = cpm_ms.astype(float).copy()
            near = np.where(in_dom & (cpm_ms <= self.deadline))[0]
        ...
        F = np.where(in_dom, F, 0.0)        # luar domain: bukan kandidat
    ...
    def select_final_roots(self, candidates):
        ...
        for cand in candidates:
            d = self.decode(cand)
            if np.any(d < self.d_min) or np.any(d > self.d_max):
                continue                     # luar domain integer: bukan solusi
