...
def process_deadline(T, tasks, res_cap, res_req, temp_params, T_normal, out_root_path):
    ...
    temp_problem_T = ProjectCrashingProblem(
        ...
        sched_cache={}, # cannot share across processes easily
    )
    ...
    if best is None or best["num_solutions"] == 0:
        print(f"  X [T={T}] Semua config gagal untuk T={T}")
        out_dir = out_root_path / f"T_{T}"
        os.makedirs(out_dir, exist_ok=True)
        summary = {
    ...
    print(f"  [T={T}] Distribusi makespan:")
    for ms in sorted(counts.keys()):
        marker = " <- DEADLINE MET" if ms <= T else ""
        print(f"    [T={T}] makespan {ms}: {counts[ms]} solusi{marker}")
    ...
    for rank, (k, root, cost, ms) in enumerate(meeting, start=1):
        ...
        try:
            ...
            print(f"    [gantt] WARNING: Gagal sol {rank} (T={T}): {exc}")
...
def run_sweep():
    ...
    # === MULTIPROCESSING ===
    # Gunakan max_workers = os.cpu_count() - 1 agar satu core tetap bebas
    ...
    with ProcessPoolExecutor(max_workers=workers) as ex:
        results = list(ex.map(process_func, target_deadlines))
