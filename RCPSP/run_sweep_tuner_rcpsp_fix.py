...
Versi MULTIPROCESSING (paralel antar-deadline).
...
# ---------------------------------------------------------------------
# PENTING: batasi thread BLAS/OpenMP per proses SEBELUM numpy/scipy
# di-import. Paralelisme kita murni antar-deadline (1 proses = 1 core),
# jadi library numerik tidak boleh ikut spawn thread sendiri
# (menghindari oversubscription: N worker x M thread BLAS).
# Karena Windows memakai metode spawn, setiap worker me-reimport module
# ini dari awal sehingga setting berikut otomatis berlaku di semua worker.
# ---------------------------------------------------------------------
...
import matplotlib
matplotlib.use("Agg")  # backend non-GUI: wajib untuk multiprocessing
...
# Cache jadwal SGS PER-PROSES. Dengan multiprocessing, tiap worker punya
# salinan cache sendiri (mulai kosong saat spawn) -- cache TIDAK dibagi
# antar worker, tapi tetap berguna karena satu worker bisa mengerjakan
# beberapa T / konfigurasi berurutan. Jadwal SGS tidak bergantung deadline
# (urutan LFT invarian terhadap pergeseran Tref) dan data task/resource
# sama sepanjang sweep, jadi reuse dalam satu proses tetap aman.
SHARED_SCHED_CACHE = {}
...
def run_soac_with_config(tasks, res_cap, res_req, T, cfg, z_star):
    ...
    t0 = time.time()
    # verbose=False: output solver dari banyak proses paralel akan saling
    # tumpang tindih di terminal dan sulit dibaca. Log ringkas per config
    # dicetak oleh grid_search_for_deadline dengan prefix [T=..].
...
def grid_search_for_deadline(tasks, res_cap, res_req, T, z_star):
    ...
    for i, cfg in enumerate(PARAM_CONFIGS):
        ...
        # Satu print = satu baris utuh, dengan prefix [T=..], supaya log
        # dari proses paralel lain tidak menyelip di tengah baris.
        if trial["gap"] is not None:
            print(f"  [T={T}] Config {i+1}/{len(PARAM_CONFIGS)} ({label}): "
                  f"Z={trial['soac_best']:.0f}, gap={trial['gap']:.0f}, "
                  f"{trial['num_solutions']} sol, {trial['elapsed']:.1f}s")
            if trial["gap"] < best_gap:
                best_gap = trial["gap"]
                best_result = trial
            if trial["gap"] <= 0:
                print(f"  [T={T}] >> GAP=0! Optimal ditemukan, skip sisa configs.")
                break
        else:
            print(f"  [T={T}] Config {i+1}/{len(PARAM_CONFIGS)} ({label}): "
                  f"no feasible solution, {trial['elapsed']:.1f}s")
...
def process_deadline(T, tasks, res_cap, res_req, temp_params, T_normal, out_root_path):
    print(f"[T={T}] Mulai grid search... (PID: {os.getpid()})")
    t_iter = time.time()
    ...
    if z_star is None:
        print(f"  [T={T}] X ILP infeasible, skip.")
        summary = {"T": T, "status": "INFEASIBLE"}
        tuning = {"T": T, "status": "INFEASIBLE", "trials": []}
        return T, summary, tuning
    print(f"  [T={T}] ILP ground truth: Z* = {z_star:.0f}")

    if z_star == 0:
        print(f"  [T={T}] Z*=0: tidak perlu crashing, skip SOAC.")
        out_dir = out_root_path / f"T_{T}"
        ...
        tuning = {"T": T, "status": "TRIVIAL", "z_star": 0, "trials": []}
        return T, summary, tuning
    ...
    if best is None or best["num_solutions"] == 0:
        print(f"  [T={T}] X Semua config gagal.")
        out_dir = out_root_path / f"T_{T}"
        os.makedirs(out_dir, exist_ok=True)
        summary = {
        ...
        with open(out_dir / "summary.json", "w") as f:
            json.dump(summary, f, indent=2)
        tuning = {"T": T, "status": "NO_SOLUTION", "z_star": z_star, "trials": trial_log}
        return T, summary, tuning
    ...
    print(f"  [T={T}] >> Best config: {best['config_label']}")
    print(f"  [T={T}] >> SOAC best: Z={soac_best:.0f}, gap={gap:.0f}, {best['num_solutions']} sol")

    counts = Counter(makespans_list)
    dist_lines = [f"  [T={T}] Distribusi makespan:"]
    for ms in sorted(counts.keys()):
        marker = " <- DEADLINE MET" if ms <= T else ""
        dist_lines.append(f"    [T={T}] makespan {ms}: {counts[ms]} solusi{marker}")
    print("\n".join(dist_lines))
    ...
    for rank, (k, root, cost, ms) in enumerate(meeting, start=1):
        ...
        try:
            ...
            print(f"    [T={T}] [gantt] WARNING: Gagal sol {rank}: {exc}")
    ...
    tuning = {"T": T, "status": "OK", "z_star": z_star, "best_config_idx": best["config_idx"], "best_config_label": best["config_label"], "best_gap": gap, "trials": trial_log}

    return T, summary, tuning

def run_sweep():
    print("=" * 80)
    print("SOAC PROJECT CRASHING SWEEP + GRID SEARCH TUNER (RCPSP) [MULTIPROCESSING]")
    print("=" * 80)
    ...
    # target_deadlines = list(range(T_normal, T_min - 1, -1))
    target_deadlines = [241, 240]  # Contoh beberapa deadline untuk testing

    # === MULTIPROCESSING ===
    # max_workers = os.cpu_count() - 1 agar satu logical processor tetap
    # bebas untuk OS (laptop tidak freeze).
    workers = max(1, (os.cpu_count() or 2) - 1)
    print(f"Menjalankan dengan ProcessPoolExecutor, {workers} workers.")

    process_func = partial(process_deadline, tasks=tasks, res_cap=res_cap, res_req=res_req,
                           temp_params=temp_params, T_normal=T_normal, out_root_path=OUT_ROOT)

    # Pakai submit + as_completed (bukan ex.map) supaya:
    # 1. Exception di satu deadline TIDAK menggagalkan seluruh sweep --
    #    deadline lain tetap terkumpul hasilnya.
    # 2. Hasil diproses begitu selesai, tidak menunggu urutan.
    results = []
    failed = []
    with ProcessPoolExecutor(max_workers=workers) as ex:
        futures = {ex.submit(process_func, T): T for T in target_deadlines}
        for fut in as_completed(futures):
            T = futures[fut]
            try:
                results.append(fut.result())
                print(f"[T={T}] Selesai. ({len(results)}/{len(target_deadlines)})")
            except Exception as e:
                failed.append(T)
                print(f"[T={T}] GAGAL dengan exception: {e!r}")

    if failed:
        print(f"\nWARNING: {len(failed)} deadline gagal: {sorted(failed, reverse=True)}")

    for T, summary, tuning in results:
        sweep_results_dict[T] = summary
        tuning_report_dict[T] = tuning
            ...
            if sum_path.exists():
                try:
                    with open(sum_path, "r") as sf:
                        ...
                        if t_val is not None and t_val not in sweep_results_dict:
