...
class ProjectCrashingProblem(BaseProblem):
    ...
    def __init__(self, tasks, deadline, params=None, unit_cube=True,
        ...
        # Precompute untuk evaluasi batch tervektorisasi (adopsi pola
        # evaluate_schedule_batch di pysne branch eksperimen-int):
        # loop Python hanya atas task (n), TIDAK atas populasi (B).
    ...
    def decode_batch(self, X):
        """(B, n) kontinu -> (B, n) durasi integer. Vektor penuh."""
    ...
    def _forward_pass_batch(self, D):
        ...
        Loop Python hanya atas task (urutan topologis); operasi per task
        divektorkan numpy di sepanjang sumbu populasi B."""
        B = D.shape[0]
